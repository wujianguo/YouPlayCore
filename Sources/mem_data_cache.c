...
static mem_cache_buf_node* find_or_new_clip(imem_cache *mem, int index) {
    ...
    if (!find) {
        ...
        n->rg.pos = ULLONG_MAX;
        n->rg.len = MEM_CACHE_BUF_SIZE_PER_CLIP;
...
static int read_data(idata_cache *cache, int index, range rg, char *buf) {
    ...
    QUEUE_FOREACH(q, &mem->clips) {
        n = QUEUE_DATA(q, mem_cache_buf_node, node);
        if (n->index == index) {
            if (is_range1_contains_range2(n->rg, rg)) {
                memcpy(buf, n->buf + rg.pos - n->rg.pos, rg.len);
                return 0;
            } else {
                range r = {n->rg.pos, MEM_CACHE_BUF_SIZE_PER_CLIP};
...
static int write_data(idata_cache *cache, int index, range rg, const char *buf) {
    ...
    mem_cache_buf_node *n = find_or_new_clip(mem, index);
    ASSERT(n->filesize != 0);
    if (n->rg.pos == ULLONG_MAX)
        n->rg.pos = rg.pos;
    
    ASSERT(RANGE_END(n->rg) == rg.pos);
    ASSERT(n->rg.len + rg.len <= MEM_CACHE_BUF_SIZE_PER_CLIP);
    memcpy(n->buf + n->rg.pos, buf, rg.len);
    n->rg.len += rg.len;
    
    if (is_range1_contains_range2(n->rg, n->want_rg))
